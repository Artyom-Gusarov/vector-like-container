...
private:
    ...
    struct block_of_memory {
        T *ptr;
        std::size_t size;

        block_of_memory(T *i_ptr, std::size_t i_size)
            : ptr(i_ptr), size(i_size) {
        }
    };
...
private:
    ...
    std::list<block_of_memory> v_allocated_blocks_size;
...
public:
    ...
    chunk_vector(chunk_vector &&other) noexcept
        : alloc_wrapper<T, Alloc, void>(std::move(other.get_alloc_ref())),
          ...
          v_allocated_blocks_size(std::move(other.v_allocated_blocks_size)) {
    }
    ...
    chunk_vector &operator=(chunk_vector &&other) noexcept {
        ...
        v_allocated_blocks_size.swap(other.v_allocated_blocks_size);
        return *this;
    }
    ...
    ~chunk_vector() {
        ...
        for (auto [block_ptr, block_size] : v_allocated_blocks_size) {
            this->deallocate(block_ptr, block_size);
        }
    }
    ...
    void reserve(size_type k) & {
        ...
        size_type cnt_of_needed_chunks = (k - capacity()) / chunk_size;
        cnt_of_needed_chunks += (k - capacity()) % chunk_size != 0;

        pointer new_block = this->allocate(chunk_size * cnt_of_needed_chunks);
        v_allocated_blocks_size.emplace_back(
            new_block, chunk_size * cnt_of_needed_chunks
        );

        for (pointer next_chunk = new_block;
             next_chunk < new_block + cnt_of_needed_chunks * chunk_size;
             next_chunk += chunk_size) {
            v_chunks.push_back(next_chunk);
        }
    }
    ...
    void shrink_to_fit() & {
        while (capacity() - v_size >= v_allocated_blocks_size.back().size) {
            this->deallocate(
                v_allocated_blocks_size.back().ptr,
                v_allocated_blocks_size.back().size
            );
            for (size_type i = 0;
                 i < v_allocated_blocks_size.back().size / chunk_size; ++i) {
                v_chunks.pop_back();
            }
            v_allocated_blocks_size.pop_back();
        }
